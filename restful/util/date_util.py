...
def get_one_day_ago_from(day=arrow.now(), zone=u'Asia/Shanghai'):
    return day.to(zone).replace(days=-1).floor('day')


def get_n_hour_ago_from(day=arrow.now(), n=10, zone=u'Asia/Shanghai'):
    return day.to(zone).replace(hours=-n).floor('hour')


def get_n_day_ago_from(day=arrow.now(), n=4, zone=u'Asia/Shanghai'):
    return day.to(zone).replace(days=-n).floor('day')


def get_last_n_week_from(day=arrow.now(), n=5, zone=u'Asia/Shanghai'):
    """
        返回上个周第一个天
    """
    return day.to(zone).replace(weeks=-n).floor('week')


def get_last_week_from(day=arrow.now(), zone=u'Asia/Shanghai'):
    """
        返回上个周第一个天
    """
    return day.to(zone).replace(weeks=-1).floor('week')


def get_last_week_to(day=arrow.now(), zone=u'Asia/Shanghai'):
    """
        返回上个周最后一天
    """
    return day.to(zone).replace(weeks=-1).ceil('week')
...
def get_last_n_month_from(day=arrow.now(), n=5, zone=u'Asia/Shanghai'):
    """
    返回上个月第一个天
    :return
    ...
    """
    return day.to(zone).replace(months=-n).floor('month')


def get_last_month_from(day=arrow.now(), zone=u'Asia/Shanghai'):
    """
    返回上个月第一个天
    :return
    date_from: 2016-01-01 00:00:00
    date_to: 2016-01-31 23:59:59
    """
    return day.to(zone).replace(months=-1).floor('month')


def get_last_month_to(day=arrow.now(), zone=u'Asia/Shanghai'):
    """
    返回上个月最后一天
    :return
    date_from: 2016-01-01 00:00:00
    date_to: 2016-01-31 23:59:59
    """
    return day.to(zone).replace(months=-1).ceil('month')


def get_quarter_range(day=arrow.now(), zone=u'Asia/Shanghai'):
    """
    返回上个月第一个天
    :return
    date_from: 2016-01-01 00:00:00
    date_to: 2016-01-31 23:59:59
    """
    return day.to(zone).span('quarter')


def get_last_n_quarter_from(day=arrow.now(), n=5,  format=u'YYYY-MM', zone=u'Asia/Shanghai'):
    """
    """
    days = []
    for i in range(n):
...
def zone_conversion(timestamp=u'1970-01-01', zone=u'Asia/Shanghai', format=u'YYYY-MM-DD'):
