...
class TimeSeriesQuery(ApiClient):
    """
    """
    def query(self, query, from_time, to_time):
        return self._api.query_timeseries(query, from_time, to_time)

    def query_rollup(self, query, from_time, to_time, desired_rollup='RAW', must_use_desired_rollup=True):
        return timeseries.query_timeseries(self._api, query, from_time, to_time, desired_rollup,
...
def do_query(query, from_time, to_time):
    ...
    responses = tsquery.query(query, from_time, to_time)
    # for response in responses:
    #     do_print(response)
    return responses


def do_query_rollup(query, from_time, to_time, desired_rollup='RAW', must_use_desired_rollup=True):
    tsquery = TimeSeriesQuery()
    responses = tsquery.query_rollup(query, from_time, to_time, desired_rollup, must_use_desired_rollup)
    return responses
